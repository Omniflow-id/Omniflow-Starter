...
exports.up = async function (knex) {
  await knex.schema.createTable("ai_analysis_settings", (table) => {
    ...
    table.string("setting_name", 100).notNullable().defaultTo("Default Analysis Settings");
    table.text("description").nullable();
    
    // Foreign key to ai_models
    table.integer("selected_model_id").unsigned().notNullable();
    table.foreign("selected_model_id").references("id").inTable("ai_models").onDelete("RESTRICT");
    
    // AI Parameters
    table.integer("max_tokens").unsigned().notNullable().defaultTo(4096);
    table.decimal("temperature", 3, 2).notNullable().defaultTo(0.10);
