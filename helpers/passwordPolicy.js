...
function getPasswordPolicy() {
  return {
    minLength: parseInt(process.env.PASSWORD_MIN_LENGTH) || DEFAULT_POLICY.minLength,
    maxLength: parseInt(process.env.PASSWORD_MAX_LENGTH) || DEFAULT_POLICY.maxLength,
    requireUppercase: process.env.PASSWORD_REQUIRE_UPPERCASE !== "false",
    requireLowercase: process.env.PASSWORD_REQUIRE_LOWERCASE !== "false", 
    requireNumbers: process.env.PASSWORD_REQUIRE_NUMBERS !== "false",
    requireSymbols: process.env.PASSWORD_REQUIRE_SYMBOLS !== "false",
    minSymbols: parseInt(process.env.PASSWORD_MIN_SYMBOLS) || DEFAULT_POLICY.minSymbols,
    minNumbers: parseInt(process.env.PASSWORD_MIN_NUMBERS) || DEFAULT_POLICY.minNumbers,
    minUppercase: parseInt(process.env.PASSWORD_MIN_UPPERCASE) || DEFAULT_POLICY.minUppercase,
    minLowercase: parseInt(process.env.PASSWORD_MIN_LOWERCASE) || DEFAULT_POLICY.minLowercase,
    maxRepeatingChars: parseInt(process.env.PASSWORD_MAX_REPEATING) || DEFAULT_POLICY.maxRepeatingChars,
    forbiddenPatterns: process.env.PASSWORD_FORBIDDEN_PATTERNS?.split(",") || DEFAULT_POLICY.forbiddenPatterns,
  };
}
...
function validatePassword(password, options = {}) {
  ...
  if (symbolCount < policy.minSymbols) {
    errors.push(`Password must contain at least ${policy.minSymbols} special character(s)`);
  }

  // Repeating characters check
  const maxRepeating = findMaxRepeatingChars(password);
  if (maxRepeating > policy.maxRepeatingChars) {
    errors.push(`Password cannot have more than ${policy.maxRepeatingChars} consecutive identical characters`);
  }
  ...
  if (options.username && lowerPassword.includes(options.username.toLowerCase())) {
  ...
  let strength = calculatePasswordStrength(password, policy);
  
  // Warnings for weak but valid passwords
  if (strength === "weak" && errors.length === 0) {
    warnings.push("Consider using a stronger password with more character variety");
  }
 ...
 */
function generateUsernameBasedPassword(username, length, policy) {
  if (!username) {
    return generateRandomPassword(length, policy);
  }

  // Clean username (capitalize first letter)
  const cleanUsername = username.replace(/[^a-zA-Z0-9]/g, "");
  const baseUsername = cleanUsername.charAt(0).toUpperCase() + cleanUsername.slice(1).toLowerCase();
  
  // Generate random numbers and symbols
  const numbers = "0123456789";
  const symbols = "!@#$%^&*";
  
  let password = baseUsername;
  
  // Add required numbers
  for (let i = 0; i < Math.max(policy.minNumbers, 2); i++) {
    password += numbers[crypto.randomInt(numbers.length)];
  }
  
  // Add required symbols
  for (let i = 0; i < Math.max(policy.minSymbols, 1); i++) {
    password += symbols[crypto.randomInt(symbols.length)];
  }
  
  // Add random characters to reach target length
...
/**
 * Generate memorable password with word-like patterns
 * @param {number} length - Target length
 * @param {Object} policy - Password policy
 * @returns {string} Generated password
 */
function generateMemorablePassword(length, policy) {
  ...
  while (password.length < length - 4) {
    ...
    password += consonants[crypto.randomInt(consonants.length)];
    password += vowels[crypto.randomInt(vowels.length)];
  ...
  // Add numbers and symbols to meet policy
  for (let i = 0; i < policy.minNumbers && password.length < length; i++) {
    password += numbers[crypto.randomInt(numbers.length)];
  }
  
  for (let i = 0; i < policy.minSymbols && password.length < length; i++) {
    password += symbols[crypto.randomInt(symbols.length)];
  }
...
/**
 * Generate completely random secure password
 * @param {number} length - Target length
 * @param {Object} policy - Password policy
 * @returns {string} Generated password
 */
function generateRandomPassword(length, policy) {
  ...
  // Ensure minimum character types are included
  ...
  if (policy.requireSymbols) {
    for (let i = 0; i < policy.minSymbols; i++) {
      password += symbols[crypto.randomInt(symbols.length)];
    }
    allChars += symbols;
  }
...
function getPasswordRequirements() {
  ...
  requirements.push(`Must be ${policy.minLength}-${policy.maxLength} characters long`);
  
  if (policy.requireUppercase) {
    requirements.push(`Must contain at least ${policy.minUppercase} uppercase letter(s)`);
  }
  
  if (policy.requireLowercase) {
    requirements.push(`Must contain at least ${policy.minLowercase} lowercase letter(s)`);
  }
  ...
  if (policy.requireSymbols) {
    requirements.push(`Must contain at least ${policy.minSymbols} special character(s)`);
  }
  
  requirements.push(`Cannot have more than ${policy.maxRepeatingChars} consecutive identical characters`);
